//! Core filesystem structures — superblock, inodes, on-disk format

use std::fs::{File, OpenOptions};
use std::io::{Read, Write, Seek, SeekFrom};

pub const MAGIC: u64 = 0x56455846_53000001; // "VEXFS\0\0\1"
...
/// On-disk superblock — first block of the filesystem
#[repr(C)]
...
/// On-disk inode — 256 bytes per file
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct DiskInode {
    pub ino: u64,
    pub size: u64,
    pub data_offset: u64,    // where on disk the file data lives
    pub created_at: u64,
    pub modified_at: u64,
    pub is_used: u8,
    pub is_dir: u8,
    pub name: [u8; 224],     // filename stored directly in inode
}
...
/// The disk manager — reads and writes to the image file
...
impl DiskManager {
    /// Open existing filesystem image
    ...
    pub fn read_inode(&mut self, index: usize) -> std::io::Result<DiskInode> {
        ...
        let inode = unsafe { *(buf.as_ptr() as *const DiskInode) };
        Ok(inode)
    }
